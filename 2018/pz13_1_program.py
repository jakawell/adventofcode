...
while len(collisions) == 0:
  ...
  for row in range(len(inputs)):
    ...
    for col in range(len(row_input)):
      ...
      last_locations[new_state_string] = inputs[new_row][new_col] # save the current empty track char before replacing the cart
        
      if is_collision: # update the new location 
        inputs[new_row] = inputs[new_row][:new_col] + "X" + inputs[new_row][new_col + 1:]
      else:
...
print("Finished. At tick " + str(tick))
for input in inputs:
  print(input)
print("First collision is at " + collisions[0].split(',')[1] + "," + collisions[0].split(',')[0])